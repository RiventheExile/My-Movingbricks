package com.my_movingbricks.tabhost;
...
import com.my_movingbricks.entity.BadgeView;
...
//            }
	 ...
	 * @param listTabModes
	 */
	public void creatTab(BaseMvpActivity activity, ArrayList<TabMode> listTabModes, IFocusChangeListener listener) {
    ...
    public void showNewMsg(int tabId, boolean isShow) {
        try {
            ...
            if (view instanceof RadioButton) {
                ...
                if (isShow) {
                    if (!badge.isShown()) {
                        badge.setCompoundDrawablesWithIntrinsicBounds(0, R.mipmap.new_tweet_icon, 0, 0);
        ...
        try {
            ...
            if (view instanceof RadioButton) {
                ...
                if (msgNum > 0) {
                    if (msgNum > 99) {
                        badge.setText("99+");
                        badge.setBackgroundResource(R.mipmap.tab_msg_num_big_bg);
                    } else {
                        badge.setText(String.valueOf(msgNum));
                        badge.setBackgroundResource(R.mipmap.tab_msg_num_bg);
                    }
                    if (!badge.isShown()) {
                        badge.show(true);
                    	if(getActivity() != null){
